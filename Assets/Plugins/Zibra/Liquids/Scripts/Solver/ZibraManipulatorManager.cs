            ...
            foreach (var manipulator in manipulators)
            {
                ...
                if (manipulator is ZibraLiquidEmitter)
                {
                    ...
                    if (emitter.PositionClampBehavior == ZibraLiquidEmitter.ClampBehaviorType.Clamp)
                    {
                        ...
                        Vector3 containerMin = containerPos - containerSize * 0.5f;
                        Vector3 containerMax = containerPos + containerSize * 0.5f;
                    ...
                    manip.AdditionalData.x = Mathf.Floor(particlesPerSec * deltaTime);
                }
        ...
        public void UpdateConst(List<Manipulator> curManipulators, List<SDFObjects.SDFCollider> sdfObjects)
        {
            ...
            foreach (var collider in sdfObjects)
            {
                ...
                if (collider is SDFObjects.ZibraNeuralCollider)
                {
                    ...
                    SDFObjects.ZibraNeuralCollider neuralCollider = collider as SDFObjects.ZibraNeuralCollider;
                    if (!neuralCollider.HasRepresentationV3)
                    {
                        Debug.LogWarning("Neural collider " + collider.gameObject.name +
            ...
            NeuralColliders = indices.NeuralColliderIndexEnd - indices.NeuralColliderIndex;
            ...
            if (ConstDataID.Count != 0)
            {
                ...
                Array.Resize<Color32>(ref Embeddings, 0);
                Array.Resize<byte>(ref SDFGrid, 0);
                ConstDataID.Clear();
            }
            ...
            foreach (var manipulator in manipulators)
            {
                ...
                if (manipulator is SDFObjects.ZibraNeuralCollider)
                {
                    SDFObjects.ZibraNeuralCollider voxcollider = manipulator as SDFObjects.ZibraNeuralCollider;

                    int EmbeddingsLength = Embeddings.Length;
                    Array.Resize<Color32>(ref Embeddings, EmbeddingsLength + voxcollider.VoxelInfo.embeds.Length);
                    Array.Copy(voxcollider.VoxelInfo.embeds, 0, Embeddings, EmbeddingsLength,
                               voxcollider.VoxelInfo.embeds.Length);

                    int SDFGridLength = SDFGrid.Length;
                    Array.Resize<byte>(ref SDFGrid, SDFGridLength + voxcollider.VoxelInfo.grid.Length);
                    Array.Copy(voxcollider.VoxelInfo.grid, 0, SDFGrid, SDFGridLength,
                               voxcollider.VoxelInfo.grid.Length);
                }
#endif
            }
        }
